...
const RUN_LINES = [
  ...
  j({ event: "transition", from: "run_impl", to: "open_draft_mr", elapsedMs: 2000, issue: { iid: 42, title: "Add X" } }),
  ...
  j({ event: "transition", from: "evaluate", to: "run_dogfood", elapsedMs: 300, issue: { iid: 42, title: "Add X" } }),
  j({ event: "transition", from: "run_dogfood", to: "merge", elapsedMs: 4000, issue: { iid: 42, title: "Add X" } }),
...
describe("projectRunStats", () => {
  ...
  it("captures run-level + issue-lifecycle facts", () => {
    ...
    expect(issue?.dogfood).toBe("pass");
  });
