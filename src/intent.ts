...
export const buildIntentBlock = (issue: IssueIntent, plan: string): string => {
  ...
  const sections = [
    ...
    "deliberate deletion, a value chosen on purpose) is NOT a bug; do not flag it",
    "as one. Review the diff for what it gets *wrong*, not for diverging from what",
    "the intent already settled.",
    "",
